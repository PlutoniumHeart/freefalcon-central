...
void RenderOTW::StartDraw(void)
{
    ...
    else
    {
        ...
        realWeather->SetGreenMode(FALSE); //JAM 10Nov03
...
void RenderOTW::SetupStates(void)
{
    ...
    else
    {
        ...
        if (DisplayOptions.m_texMode == DisplayOptionsClass::TEX_MODE_DDS/* && TheTerrTextures.lightLevel < 0.5f*/)
        {
            ...
            state_mid = STATE_MULTITEXTURE;
        }
        else
        {
            state_fore = STATE_LANDSCAPE_GOURAUD;
...
void RenderOTW::SetTunnelPercent(float percent, DWORD color)
{
    ...
    // RV - Biker - Give little more alpha for blackout
    //tunnelAlphaWidth = percent*PercentScale;
    //tunnelSolidWidth = tunnelAlphaWidth - PercentBlend;
    tunnelAlphaWidth = min(percent * 2.0f, 1.05f);
    tunnelSolidWidth = percent * 2.0f - 0.95f;
}
...
// Used to speed up Map View or entering 3D
...
void RenderOTW::DrawScene(const Tpoint *offset, const Trotation *orientation)
{
    ...
    */
    ...
    /*switch(containingList){
     ...
     default : {DWORD DrawOrder[]={LAYER_NODRAW, LAYER_NODRAW, LAYER_NODRAW, LAYER_NODRAW, LAYER_NODRAW, LAYER_TOP };
     TheDXEngine.DX2D_SetDrawOrder(DrawOrder);}
...
void RenderOTW::ComputeVertexColor(TerrainVertex *vert, Tpost *post, float distance, float x, float y)
{
    ...
    // Disable as default
    TheDXEngine.LinearFog(false);

    // ***************** COBRA - RED - SINGLE OTW RENDERER / THE NVG VERSION *****************************
    if (TheTimeOfDay.GetNVGmode())
    {
        float alpha, fog;
        ...
        else if (distance > haze_start + haze_depth + 3000)
        {
            vert->RenderingStateHandle = state_far;
        }
    ...
    if (realWeather->weatherCondition > FAIR)
    {
        ...
        // check for lightining
        if (realWeather->isLightning)
        {
    ...
    else
    {
        ...
        if (DisplayOptions.bSpecularLighting) // Terrain shading
        {
            float iDiff = 0.f;
